...
void ManyParamFunction2(const int& a, const int& b, const int& c, const int& d, const int& e) {
}
...
void IfFunction() {
	...
	// 한줄로 변경됩니다.
	if (true == cond1 && true == cond2 && true == cond3 && true == cond4) {
	}

	// if문 내에서 함수 호출 시 파라메터 모두 한줄로 수정됩니다.
	if (false == ManyParamFunction3(1, 2, 3, 4, 5)) {
	}
}
...
class BreakInheritanceList2 : public AfterColon, public Base {
};

class BreakInheritanceList3 : public AfterColon, public Base {
};

class BreakInheritanceList4 : public AfterColon, public Base {
};
